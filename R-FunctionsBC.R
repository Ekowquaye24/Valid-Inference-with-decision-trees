...
cart <- function(formula, data, method="anova", control=NULL,
	...
	if (size.selection=="0SE") {
		...
		cp.best <- tre0$cptable[opt, "CP"]; # print(cp.best) 
	...
	leaf.info$node <- n.leaf$Group.1
	# OUTPUT
	btree.size <- NROW(leaf.info)
	list(leaf=leaf.info, btree=best.tree, cp=best.cp, size=btree.size)
}
...
send.down <- function(fit.cart, data, yname="y"){
	...
	# DEVIANCE COMPUTED W.R.T. EACH TEST SAMPLE MEAN
	# n.test <- leaf.test$x
	# dev.test <- aggregate(dat.tmp$y, by=list(dat.tmp$node), FUN=var)$x *(n.test-1)
	yval.test <- aggregate(dat.tmp$y, by=list(dat.tmp$node), FUN=mean)$x
	sd.test <- aggregate(dat.tmp$y, by=list(dat.tmp$node), FUN=sd)$x
	# SUMMARIZE RESULTS
	# leaf.test <- cbind(leaf.test, dev.test, yval.test, sd.test)
	# names(leaf.test) <- c("node", "n.test", "dev.test", "ybar.test", "sd.test")
	leaf.test <- cbind(leaf.test, yval.test, sd.test)
	names(leaf.test) <- c("node", "n.test", "ybar.test", "sd.test")
	# leaf.info <- merge(leaf, leaf.test, by="node", all.x = TRUE)
